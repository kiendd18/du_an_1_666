                            ...
                            <div class="bill_container_detail">
                                ...
                                <?php foreach ($product_name_result as $key => $product): ?>
                                    ...
                                    <div style="display: flex; align-items: center; justify-content: space-between"
                                        ...
                                        <div style="display: flex; align-items: center; gap: 15px;">
                                            ...
                                            <div>
                                                ...
                                                <div style="margin-top:8px;">
                                                    ...
                                                    <span style="margin-left: 15px" class="color_name">SỐ LƯỢNG:
                                        ...
                                        <div>
                                            <a href="/du_an1/product_detail&product_id=<?= $product['product_id'] ?>"
                                                style="width: 150px" class="btn_view_product">
                                ...
                                <div class="bill_status_and_total_price">
                                    <span class="product_total__price">Tổng tiền:
                                        <strong><?= formatMoney($value['total_price']); ?></strong></span>
                                    <div class="bill_status">
                                        <?php $status_result = select_status_by_id($status_id); ?>

                                        Trạng thái:
                                        <span class="status_name" status="<?= $status_id ?>"
                                            style="color: <?= $status_id == 1 || $status_id == 5 ? "#e03033;" : "#26820b;"; ?>">
                                            <?= $status_result['status'] ?></span>
                                    </div>
                                </div>
                                <?php if ($status_id == 1): ?>
                                    <button type="button" order_id="<?= $value['order_id'] ?>" status_id="5" class="cancel_order">Hủy
                                        đơn hàng</button>
                                <?php endif ?>
                                <?php if ($status_id == 3): ?>
                                    <button type="button" order_id="<?= $value['order_id'] ?>" status_id="4" class="receive">Đã nhận
                                        được hàng</button>
                                <?php endif ?>
...
<?php else: ?>
    <div class="empty_session">
        ...
        <div style="display: flex;justify-content: center;margin-top:18px">
            <button type="button" id="getCustomerInfo" style="cursor: pointer;display:block;">Nhập email và số điện
                thoại</button>
        </div>
        <div class="get_anomyous_customer_info"
            style="width: 400px;margin: 0 auto;border: 1px solid #dedede;padding: 16px;display: none;margin-top: 12px;">
            <form action="/du_an1/get_anomyous_customer_info" method="post">
                ...
                <input type="email" name="receiver_email" id="receiver_email" required
                    style="display: block;width: 100%;padding: 10px;margin-bottom: 20px;border-radius: 5px;border: 1px solid #ccc;font-size: 16px;">
                <label for="receiver_numberPhone">Số điện thoại:</label> <br>
                <input type="text" name="receiver_number_phone" id="receiver_numberPhone"
                    style="display: block;width: 100%;padding: 10px;margin-bottom: 20px;border-radius: 5px;border: 1px solid #ccc;font-size: 16px;"
                    required>
